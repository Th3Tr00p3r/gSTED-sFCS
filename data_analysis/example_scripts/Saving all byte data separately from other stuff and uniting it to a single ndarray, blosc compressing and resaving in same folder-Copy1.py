...
MEAS_TEMPLATE = "bp300YOYO_Fresh_HighConcentration_500nW_angular_exc_140837_"

data_label_kwargs = {
    "TEST": dict(
        date=DATE,
        confocal_template=f"{MEAS_TEMPLATE}*.pkl",
...
total_byte_data = np.hstack(
    tuple([np.hstack((byte_data, np.array([254], dtype=np.uint8))) for byte_data in byte_data_list])
)
...
new_file_path = file_path.with_name(f"{MEAS_TEMPLATE}{n_files + 999}.pkl")
