...
class PhotonData:
    ...
    def convert_counts_to_images(self, counts, ao, scan_params: dict, um_v_ratio) -> None:
        ...
        # remove counts from outside limits
...
def calc_plane_image_stack(counts_stack, eff_idxs, pxls_per_line):
    ...
    for i in range(pxls_per_line):
        matching_counts_stack = counts_stack[:, eff_idxs == i, :]
        image_stack[i, :, :] = matching_counts_stack.sum(axis=1)
        norm_stack[i, :, :] = matching_counts_stack.shape[1]

    return image_stack.transpose(1, 0, 2), norm_stack.transpose(1, 0, 2)
